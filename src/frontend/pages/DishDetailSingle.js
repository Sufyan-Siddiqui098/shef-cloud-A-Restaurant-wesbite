...
export const DishDetailSingle = () => {
  ...
  // selected delivery date, and slot
  ...
  // Function to check available days including today in the next 7 to 14 days
  function getAvailableDates(
    ...
    // This array will hold the available dates
    let availableDates = [];

    // Iterate over the next 14 days starting from today or a custom start day
    for (let i = startDaysFromNow; i < startDaysFromNow + totalDays; i++) {
      // Create a new date for the current iteration
      ...
      // Check if that day of the week is available in the `availability` object
      if (availability[`is_${daysOfWeek[dayOfWeek].toLowerCase()}`] === 1) {
        availableDates.push(futureDate.toDateString()); // Push the formatted date to the availableDates array
      }
    }
    ...
    const fetchSingleDish = async () => {
      try {
        ...
        if (getNext14AvailableDays) {
          setAvailableDates(getNext14AvailableDays);
        }
      } catch (error) {
  ...
  const handleAddToCart = () => {
    ...
    // --- add delivery date and time under chef object
    const chef = {
      ...dish.chef,
  ...
  useEffect(() => {
    ...
    cartItem?.forEach((chef, chefIndex) => {
      if (
        chef.id === dish?.chef?.id &&
        chef.delivery_date === selectedDeliverDateAndSlot.delivery_date &&
  ...
  return (
    <>
      <div>
        ...
        {isFetching && (
          <div className="fixed top-0 bottom-0 h-full w-[100vw] flex justify-center items-center bg-white bg-opacity-80 z-50">
            <h4 className="text-headGray">Loading...</h4>
          </div>
        )}

        <div className="container mx-auto my-6 lg:px-2 px-4">
          {/* Back Button */}
